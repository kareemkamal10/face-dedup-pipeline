...
3. تحميل وتشغيل الدفعات بأسلوب Pipelining:
   - الدفعة الكبيرة الأولى (20,000 عنصر) بتتحمل **كاملة** الأول، بتوازي ثابت 100 صورة
     في نفس اللحظة (بغض النظر عن عدد الصور لكل عنصر أو حجمها).
   - بعد كده، وإحنا بنعالج الدفعة الحالية على GPU (كشف وجه + متوسط embedding)،
     الدفعة اللي بعدها بتتحمل في الخلفية بالتوازي في نفس الوقت.
   - بالطريقة دي الـ GPU مبيستناش التحميل، والتحميل مبيستناش الـ GPU — الاتنين
     شغالين مع بعض قد الإمكان.
4. لكل عنصر: كشف الوجه في كل صورة من صوره، وأخذ أكبر وجه (الأقرب للكاميرا) لو فيه أكتر من وجه،
   وحساب متوسط الـ embeddings لكل صور نفس الشخص.
...
> **ملاحظة مساحة**: بما إن الدفعة الكبيرة كاملة (20,000 عنصر، يمكن 60-80 ألف صورة)
> بتتحمل قبل المعالجة، مساحة `/kaggle/temp` المستخدمة وقت التحميل هتبقى أكبر من
> قبل (دفعة كاملة بدل 250 عنصر بس). لو مساحة temp عندك محدودة جدًا، قلل
> `OUTER_BATCH_SIZE` في `config.py`.
